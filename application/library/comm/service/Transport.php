...
class Comm_Service_Transport
{
     ...
     * 执行方法
     * @static
     * @param array $requestList 请求信息 new Comm_Service_Request()
    ...
    {
        Comm_Service_MultiCurl::instance()->open();
        Comm_Service_MultiCurl::instance()->send($requestList);
        $responseList = Comm_Service_MultiCurl::instance()->exec();
        Comm_Service_MultiCurl::instance()->close();
        return $responseList;
    }
    ...
    private static function curlExec($requestList)
    {
        ...
        foreach ($requestList as $key => $request) {
            Comm_Service_Curl::instance()->open();
            Comm_Service_Curl::instance()->send($request);
            $responseList[$key] = Comm_Service_Curl::instance()->exec();
            Comm_Service_Curl::instance()->close();
        }
        return $responseList;
    }
}